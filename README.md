...
__Go map: o(1) How so?__  In truth go map converts the fist to bytes into 2 uint8, thoes 2 bytes are useed a keys in a 2 tier tree, the remaing bytes then hit the o(1) or full scan.  This hits the sweet spot on most use cases.  The side effect is, keys are never going to be ordered.
